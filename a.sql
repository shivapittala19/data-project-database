...
SELECT NO_EXTRA.BOWLER,ROUND((CAST((no_extra.runs_conceded + extra.runs_conceded) AS DECIMAL(10, 2)) / CAST( no_extra.balls AS DECIMAL(10, 2))) * 6,2) 
AS ECONOMY
FROM
	(SELECT BOWLER,
		...
		GROUP BY BOWLER) AS NO_EXTRA
LEFT JOIN
	(SELECT BOWLER,
			SUM(WIDE_RUNS + NOBALL_RUNS) AS RUNS_CONCEDED
		FROM DELIVERIES
		WHERE MATCH_ID BETWEEN 517 AND 591
		GROUP BY BOWLER) AS EXTRA ON NO_EXTRA.BOWLER = EXTRA.BOWLER
ORDER BY ECONOMY
LIMIT 10;
